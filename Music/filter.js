...
async execute(message, args, client) {
    ...
    if (!message.member.voice.channel)
      return attentionembed(message, "Lütfen İlk Bir Kanala Girin")
    ...
    switch (args[0]) {
      ...
      default:
        ...
        .setColor("RANDOM")
        .setTitle("Geçerli Bir Filitre Değil, Aşağıdakilerden Birini Kullanmayı Deneyin")
        .setDescription(`
        \`bassboost\`
        \`8D\`
        \`vaporwave\`
        ...
        \`pulsator\`
        \`subboost\`
        \`clear\`   ---  Tüm Filitreleri Kaldırır`)
        .setFooter(`Örnek: ${PREFIX}filitre bassboost`)
        )
        break;
    }
    //set choice to zero
    ...
    try {
      ...
      message.channel.send(new MessageEmbed()
      .setColor("RANDOM")
      .setAuthor("Uygulanıyor: " + args[0], "https://cdn.discordapp.com/emojis/769935094285860894.gif")).then(msg =>{
