...
async execute(message, args, client) {
    ...
    if (serverQueue && channel !== message.guild.me.voice.channel)
      return attentionembed(message, `Benimle Aynı Ses Kanalında Olman Gerekiyor`);
    //If no args return
    if (!args.length)
      return attentionembed(message, `Kullanım: ${message.client.prefix}play <YouTube URL | Video İsmi | Soundcloud URL>`);
    //react with approve emoji
    message.react("✅").catch(console.error);
    //get permissions and send error if bot doesnt have enough
    const permissions = channel.permissionsFor(message.client.user);
    if (!permissions.has("CONNECT"))
      return attentionembed(message, "Senin Kanalına Girebilmem İçin Yeterli İznim Yok");
    if (!permissions.has("SPEAK"))
      return attentionembed(message, "Senin Kanalında Konuşmaya İznim Yok");
      ...
      if (serverQueue) {
        ...
        if (urlValid) { //send searching link
          message.channel.send(new MessageEmbed().setColor("RANDOM")
            .setDescription(`**💢 Müzik Aranıyor 🔍 [\`LINK\`](${args.join(" ")})**`))
        //if not
        }
        else { //send searching TITLE
          message.channel.send(new MessageEmbed().setColor("RANDOM")
            .setDescription(`**💢 Müzik Aranıyor 🔍 \`${args.join(" ")}\`**`))
        }
      } else {
        ...
        //send join message
        message.channel.send(new MessageEmbed().setColor("RANDOM")
          .setDescription(`**👍 Senin Olduğun Kanala Başarıyla Katıldım \`${channel.name}\` 📄 bound \`#${message.channel.name}\`**`)
    ...
    if (serverQueue) {
      ...
      const newsong = new MessageEmbed()
        ...
        .setThumbnail(thumb)
        .setURL(song.url)
        .setDescription(`\`\`\`Sıraya Başarıyla Katıldı.\`\`\``)
        .addField("Çalınırken Ki Geçen Zaman:", `\`${estimatedtime}\``, true)
        .addField("Sıradaki Pozisyon", `**\`${serverQueue.songs.length - 1}\`**`, true)
    ...
    try {
      ...
      return attentionembed(message, `Kanala Katılınamıyor: ${error}`);
    }
  }
};
