...
async execute(message,args,client) {
    ...
    if (!args.length)
      return attentionembed(message,`Kullanım: ${message.client.prefix}${module.exports.name} <Video İsmi>`)
    //if there is already a search return error
    if (message.channel.activeCollector)
      return attentionembed(message,"Şaun Bir Arama Sürüyor");
    //if the user is not in a voice channel return error
    if (!message.member.voice.channel)
      return attentionembed(message,"Lütfen İlk Bir Kanala Giriniz")
    ...
    .setAuthor("Aranıyor...", "https://cdn.discordapp.com/emojis/757632044632375386.gif?v=1")
    .setColor("#f300e5")
    ...
    let resultsEmbed = new MessageEmbed()
      ...
      .setColor("RANDOM")
      .setFooter("Favori Sayınız İle Cevam Verin", client.user.displayAvatarURL() )
    ...
    try {
      ...
      if(response === "error"){
        //send error message
        attentionembed(message,"Lütfen Doğru Emojiyi Seçiniz");
