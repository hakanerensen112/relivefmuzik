...
module.exports = {
  async play(song, message, client, filters) {
    ...
    const { PRUNING, SOUNDCLOUD_CLIENT_ID } = require("../util/configv2");
  ...
  let thumb;
    ...
    try {
      const newsong = new MessageEmbed()
        ...
        .setColor("RANDOM")
        .setThumbnail(thumb)
        .setFooter(`Tarafından İstendi: ${message.author.username}#${message.author.discriminator}`, message.member.user.displayAvatarURL({ dynamic: true }))
        .addField("Zaman:", `\`${song.duration} Dakika\``, true)
    ...
    collector.on("collect", async (reaction, user) => {
      ...
      switch (reaction.emoji.name) {
        ...
        let nowPlaying = new MessageEmbed()
          .setTitle("Şuan Oynatılıyor")
          .setDescription(`[**${song.title}**](${song.url})`)
          .setThumbnail(song.thumbnail.url)
          .setColor("RANDOM")
          .setFooter("Kalan Zaman: " + new Date(left * 1000).toISOString().substr(11, 8));
          //if its a stream
          if(ms >= 10000) {
        ...
        case "⏭":
          ...
          const skipembed = new MessageEmbed().setColor("RANDOM").setAuthor(`${user.username} Müzik Geçiliyor`, "https://cdn.discordapp.com/attachments/748633941912584333/753201474691137647/next.png")
        ...
        case "📑":
          ...
          try {
            ...
            if (!lyrics) lyrics = `Bu Müzik İçin Bir Söz Bulunamadı ${queue.songs[0].title}.`;
          } catch (error) {
            lyrics = `Bu Müzik İçin Bir Söz Bulunamadı ${queue.songs[0].title}.`;
          }
        ...
        case "⏯":
          ...
          if (queue.playing) {
            ...
            const pausemebed = new MessageEmbed().setColor("RANDOM")
              .setAuthor(`${user.username} Müzik Durduruldu`, "https://cdn.discordapp.com/emojis/769912238236106793.png")
            queue.textChannel.send(pausemebed).catch(console.error);
          } else {
            queue.playing = !queue.playing;
            queue.connection.dispatcher.resume();
            const playembed = new MessageEmbed().setColor("RANDOM")
              .setAuthor(`${user.username} Müzik Sürdürülüyor`, "https://cdn.discordapp.com/emojis/769912238236106793.png")
        ...
        case "🔄":
          ...
          const loopembed = new MessageEmbed().setColor("RANDOM")
            .setAuthor(`Döngü şuan ${queue.loop ? " enabled" : " disabled"}`, "https://cdn.discordapp.com/emojis/769913064194834511.png")
        ...
        case "⏹":
          ...
          if (!canModifyQueue(member)) return;
          queue.songs = [];
          const stopembed = new MessageEmbed().setColor("RANDOM").setAuthor(`${user.username} Müzik Durduruldu`, "https://cdn.discordapp.com/emojis/769915194066862080.png")
